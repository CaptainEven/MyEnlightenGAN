...
class SingleModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            ...
            self.netD_A = networks.define_D(opt.output_nc,
                                            ...
                                            False)
            if self.opt.patchD:
                self.netD_P = networks.define_D(opt.input_nc,
                                                opt.ndf,
                                                opt.which_model_netD,
                                                opt.n_layers_patchD,
                                                opt.norm,
        ...
        load_ckpt = not self.isTrain or opt.continue_train
        print("Load check point: ", load_ckpt)
        if load_ckpt:
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            if self.opt.noise > 0:
                self.noise = torch.cuda.FloatTensor(self.real_A.size()).normal_(mean=0, std=self.opt.noise / 255.0)
    ...
    def predict(self, pad_left, pad_right, pad_top, pad_bottom):
        """
        :param pad_left:
        :param pad_right:
        :param pad_top:
        ...
        with torch.no_grad():
            ...
            if self.opt.noise > 0:  # do not go this way
                self.noise = torch.cuda.FloatTensor(self.real_A.size()).normal_(mean=0, std=self.opt.noise / 255.0)
                self.real_A = self.real_A + self.noise
            if self.opt.input_linear:  # do not go this way
                self.real_A = (self.real_A - torch.min(self.real_A)) / (torch.max(self.real_A) - torch.min(self.real_A))

            # print(np.transpose(self.real_A.item().cpu().float().numpy(),(1,2,0))[:2][:2][:])
            if self.opt.skip == 1:  # go this way
                # ## use gray as self attention
                # self.fake_B, self.latent_real_A = self.netG_A.forward(self.real_A, self.real_A_gray)

                ## do not use gray as self attention
        ...
        ## Padding tensor back
        self.real_A = pad_tensor_back(self.real_A, pad_left, pad_right, pad_top, pad_bottom)
        self.fake_B = pad_tensor_back(self.fake_B, pad_left, pad_right, pad_top, pad_bottom)
        # self.latent_real_A = pad_tensor_back(self.latent_real_A, pad_left, pad_right, pad_top, pad_bottom)
        self.real_A_gray = pad_tensor_back(self.real_A_gray, pad_left, pad_right, pad_top, pad_bottom)

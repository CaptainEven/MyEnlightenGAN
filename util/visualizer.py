...
class Visualizer():
    ...
    def save_images(self, webpage, visuals, img_path, ext=".png", save_pair=True):
        """
        :param webpage:
        :param visuals:
        :param img_path:
        ...
        for label, img_np in visuals.items():
            ...
            if save_pair:
                img_name = '{:s}{:s}'.format(name, ext)
            else:
                img_name = '{:s}_{:s}{:s}'.format(name, label, ext)
            save_path = os.path.join(save_dir, img_name)

            if save_pair:
                img_ori = cv2.imread(img_path[0], cv2.IMREAD_COLOR)
                h, w, c = img_ori.shape
                ...
                util.save_image(img_np, save_path)
